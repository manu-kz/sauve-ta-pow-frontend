...
import { keepUsername, keepToken, showLoginProcess } from "../../reducers/user";
...
export default function LoginModal({ navigation }) {
  ...
  async function login() {
    const rawRes = await fetch(
      "https://sauve-ta-pow-backend.vercel.app/users/signin",
      {
        method: "POST",
  ...
  return (
    <Modal animationType="slide" transparent={true} visible={loginModal}>
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        ...
        <Text
          onPress={() => {
            dispatch(setLoginModal(false)), dispatch(setSignUpModal(true));
          }}
        >
          ou créer un compte
        </Text>
